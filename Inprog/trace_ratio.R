...
#' adapted from:
#' 
#' https://github.com/kyoustat/maotai/blob/master/R/trio.R
#' 
...
#' 
#' @export
trio <- function(A, B, dim=2, method=c("2003Guo","2007Wang","2009Jia","2012Ngo"), maxiter=1000, eps=1e-10){
  ...
  if (nrow(A) != nrow(B)){
    stop("trace_ratio : all input matrices should be of same size.")
  }
  
  
  # 2009 Jia's note : B should have rank >= (m-d)
  if (as.integer(Matrix::rankMatrix(B))<(nrow(B)-dim)){
    warning("trace_ratio : null space of 'B' is excessive. trace ratio value may diverge.")
  }
  ...
  output = list()
  output$v = V
  output$values = sum(diag(t(V)%*%A%*%V))/sum(diag(t(V)%*%B%*%V))
  return(output)
}
